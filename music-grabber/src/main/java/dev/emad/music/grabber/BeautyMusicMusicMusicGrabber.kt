...
class BeautyMusicMusicMusicGrabber : MusicGrabber() {
    override fun grab(url: String): Flow<MusicInformation> = channelFlow {
        val document = Jsoup.connect(url).get()
        val element = document.select("div").firstOrNull { element -> element.hasClass("col-xs-12 player") }
        ...
        if (downloadUrl != null) {
            val musicInformation = MusicInformation(
                source = MusicSource.BEAUTY_MUSIC,
                pageUrl = url,
